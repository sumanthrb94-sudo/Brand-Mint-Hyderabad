...
| `login.html` | Google sign-in, three states by URL: `?tier=<id>` (a buyer — "Almost there"), `?for=toolkit` (the free PDFs), and bare (a client — "Sign in to your project"; also where `?next=`, `?signedout=1`, `?denied=1` land). The nav's **Client login** is the bare state. Calls `recordSignup()` after auth; the toolkit email only fires for the first two |
...
| `downloads/` | The three free PDFs (checklist, catalogue template, scope worksheet) — branded A4, nothing else. Sources in `downloads/src/`, regenerate with `NODE_PATH=$(npm root -g) node downloads/src/render.cjs` |
