  ...
  return (
    <main className="relative grid min-h-screen place-items-center overflow-hidden bg-[#f6f0e6] px-5 py-10">
      ...
      <section className="relative w-full max-w-md rounded-[28px] border border-[#d7cbbb] bg-[#fffdf8]/95 p-7 shadow-[16px_18px_0_#e5d8c5] sm:p-9">
        ...
        <div className="mt-8"><BrandMark /><p className="eyebrow mt-8">Secure access</p><h1 className="mt-3 font-display text-4xl tracking-[-0.055em] text-[#14392d]">Sign in to continue.</h1><p className="mt-3 text-sm leading-6 text-[#668078]">Use the approved Google account to access Brand Mint Studios. CEO access is granted only to the configured Brand Mint CEO account.</p></div>
        <Button onClick={signInWithGoogle} disabled={busy} className="mt-7 h-12 w-full rounded-full bg-[#103c2e] text-white hover:bg-[#0b3024]">{busy ? <Loader2 className="h-4 w-4 animate-spin" /> : <ShieldCheck className="h-4 w-4" />}Continue with Google</Button>
        <p className="mt-4 text-center text-[11px] leading-5 text-[#789087]">Google sign-in is the only access method for this workspace.</p>
