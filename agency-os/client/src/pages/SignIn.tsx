...
import { firebaseAuthErrorMessage, safeReturnPath, startFirebaseGoogleLogin } from "@/lib/firebase";
...
export default function SignIn() {
  ...
  useEffect(() => {
    ...
    setError("Google sign-in completed, but Brand Mint could not confirm your account. Retry the account check once. If it persists, the server configuration needs attention.");
  ...
  return (
    <main className="relative grid min-h-screen place-items-center overflow-hidden bg-[#f6f0e6] px-5 py-10">
      ...
      <section className="relative w-full max-w-md rounded-[28px] border border-[#d7cbbb] bg-[#fffdf8]/95 p-7 shadow-[16px_18px_0_#e5d8c5] sm:p-9">
        ...
        <div className="mt-8"><BrandMark /><p className="eyebrow mt-8">One account, one entry point</p><h1 className="mt-3 font-display text-4xl tracking-[-0.055em] text-[#14392d]">{accessCopy.signedOutTitle}</h1><p className="mt-3 text-sm leading-6 text-[#668078]">{accessCopy.signedOutDetail}</p></div>
        ...
        <p className="mt-4 text-center text-[11px] leading-5 text-[#789087]">There is no separate sign-up. Your approved account determines the workspace you open.</p>
