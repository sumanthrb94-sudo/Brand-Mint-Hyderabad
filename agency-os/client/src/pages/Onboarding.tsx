...
function Field({ label, children }: { label: string; children: React.ReactNode | ((id: string) => React.ReactNode) }) {
  ...
  return (
    <div className="space-y-2">
      <Label htmlFor={id} className="text-xs font-bold uppercase tracking-[0.12em] text-[#5e766d]">{label}</Label>
...
/**
 * A set of related checkboxes is named by a group, not by a label — a label can
 * only name a single control, and htmlFor pointing at a wrapping div names
 * nothing at all.
 */
function FieldGroup({ label, children }: { label: string; children: React.ReactNode }) {
  ...
  return (
    <div className="space-y-2" role="group" aria-labelledby={id}>
      <span id={id} className="flex items-center gap-2 text-xs font-bold uppercase tracking-[0.12em] text-[#5e766d]">{label}</span>
      {children}
    </div>
  );
}
