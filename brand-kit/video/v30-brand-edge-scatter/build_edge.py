"""
Brand Mint Studios — Brand Story Ident (v30 / edge scatter)
...
OUT_DIR = HERE / "out"
OUT_MP4 = OUT_DIR / "brandmint-edge-60fps.mp4"
...
def make_particles() -> List[P]:
    ...
    for i in range(N_STREAM):
        ...
        born = 3.10 + u * 2.0
        ps.append(P(x=start_x, y=start_y, z=lerp(0.4, -0.2, u),
                    vx=0, vy=0, vz=0,
                    tx=x, ty=y,
...
def step(t: float):
    ...
    for i, p in enumerate(PARTICLES):
        ...
        # Lead + orbiter dots share the same shot-1 behavior: each drifts
        # from the canvas mark center to its assigned edge target.
        if p.role in ("lead", "orbiter"):
            if t < 3.0:
                # Edge index: lead (seed=0.0) -> top (0); orbiters with
                # seed 0.25/0.50/0.75 -> right(1)/bottom(2)/left(3).
                edge_i = int(round(p.seed * 4)) % 4
                ex, ey = EDGE_TARGETS[edge_i]
                age = t - p.born_t
                appear = ease_out_cubic(clamp01(age / 2.4))
                # Subtle micro-wobble while drifting so the dots feel alive
                ...
                # Shots 2-3: migrate back from the edge to a lattice slot.
                idx = (i + 5) % len(LATTICE)
                ...
                # Shot 4+: travel to a nearby M target.
                idx = (i + 5) % len(M_TARGETS)
                tx, ty = M_TARGETS[idx]
                k = 0.10; damp = 0.84
                p.vx = (p.vx + (tx - p.x) * k) * damp
                p.vy = (p.vy + (ty - p.y) * k) * damp
        ...
        if t < 9.5:
            ...
            # Stiffer spring (0.10 vs prev 0.065) so the M is fully
            # locked by ~11.5s instead of ~13s.
            k = 0.10 * smoothstep(9.5, 10.5, t); damp = 0.82
            dx, dy = p.tx - p.x, p.ty - p.y
            dist = math.hypot(dx, dy) + 1e-6
            perp_strength = min(0.5, dist / 300.0) * 0.008
            sign = 1.0 if (i % 2 == 0) else -1.0
            p.vx = (p.vx + dx * k - dy * perp_strength * sign) * damp
            p.vy = (p.vy + dy * k + dx * perp_strength * sign) * damp
            # Snap z toward 0 once the M is forming so all locked dots
            # render at the same size for uniform symmetric weight.
            p.vz = (p.vz + (-p.z * 0.20)) * 0.88
            p.x += p.vx; p.y += p.vy; p.z += p.vz
        else:
            # SHOTS 5+: lock hard onto M target, z=0 for uniform size.
...
def main():
    print(f"v30 edge-scatter · {FPS}fps · {TOTAL_S}s · {N_PARTICLES} dots")
    render_all_frames()
    mux()
