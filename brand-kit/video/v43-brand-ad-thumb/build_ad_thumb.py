"""
Brand Mint Studios — Brand Ad (v43 / held-thumbnail intro)

16.0s · 60fps · 1080x1920 vertical · MUSIC: mr_claps_fashion_abstract.

Change from v42: PREPENDS a 1.0s held thumbnail composition at the
start so the first frame is a clean brand poster (M monogram +
BRAND MINT + compound. + URL + CTA pill). The thumbnail dissolves
out smoothly between t=0.6 and t=1.0s, and the 6-scene ad runs from
t=1.0 to t=16.0s. Total video length 16.0s.

Audio: delayed 0.1s via ffmpeg -itsoffset 0.1. Music begins playing
under the held thumbnail (at video t=0.1s) and rides through the ad.
The 0.1s of pure silence at the very start is intentional — gives
the held thumbnail a moment to register before the music kicks in.

All v42 scene logic preserved; only the time offset changes.
...
W, H = 1080, 1920
FPS = 60
THUMBNAIL_HOLD_S = 1.0       # held brand-poster at the start
AUDIO_OFFSET_S   = 0.1       # music begins 0.1s into the video
AD_S = 15.0                  # the v42 ad runs for 15s after the hold
TOTAL_S = THUMBNAIL_HOLD_S + AD_S    # = 16.0
...
OUT_DIR = HERE / "out"
OUT_MP4 = OUT_DIR / "brandmint-ad-thumb-60fps.mp4"
...
# ---------- Frame composer ----------
def render_thumbnail_overlay(video_t: float) -> str:
    """Held brand-poster composition for the first ~1.0s. Fades out
    0.6 -> 1.0s. Composition mirrors the static thumbnail PNG used as
    the social-media cover: M monogram + BRAND MINT + STUDIOS kicker +
    compound. tagline + URL + 'Start a project ->' mint pill."""
    if video_t > 1.05: return ""
    a = 1.0 - smoothstep(0.6, 1.0, video_t)
    if a <= 0.01: return ""
    ...
    return f"""
      <rect width="{W}" height="{H}" fill="{INK}" opacity="{a:.3f}"/>
      <g opacity="{a:.3f}">
        <circle cx="{cx}" cy="{mark_cy}" r="{mark_s*0.66:.1f}"
                fill="url(#markGlow)" opacity="0.55"/>
        <circle cx="{cx}" cy="{mark_cy}" r="{mark_s*0.5:.1f}" fill="url(#bmDisc)"/>
        <path d="M{cx-mark_s*0.219:.2f} {mark_cy+mark_s*0.187:.2f}
        ...
        <rect x="{cx - 220:.1f}" y="{cta_y - 38:.1f}" width="440" height="76"
              rx="38" fill="{MINT_3}"/>
        <text x="{cx}" y="{cta_y + 12:.1f}" font-family="{FONT_DISPLAY}"
...
def svg_for_frame(f: int) -> str:
    video_t = f / FPS
    # v43: held thumbnail occupies the first THUMBNAIL_HOLD_S; the ad's
    # internal t (and scene timings) start counting from then.
    t = max(0.0, video_t - THUMBNAIL_HOLD_S)
    # Tiny micro-flash on scene boundary for a tactile cut feel. Boundaries
    # are the 5 internal scene cuts (SCENE_STARTS[1:]) — checked in ad t.
    flash = ""
    if video_t >= THUMBNAIL_HOLD_S:
  ...
  {render_thumbnail_overlay(video_t)}
...
def mux():
    ...
    # The ad starts at video t = THUMBNAIL_HOLD_S. Music begins at
    # video t = AUDIO_OFFSET_S. So audio plays under the thumbnail for
    # (THUMBNAIL_HOLD_S - AUDIO_OFFSET_S) seconds (~0.9s), then under
    # the ad for the remaining 15s. Audio fades out at the end.
    ...
    cmd = [
        ...
        # Trim the audio's intro AND offset the result by AUDIO_OFFSET_S
        # in the timeline. -itsoffset must come BEFORE the -i it applies to.
        "-ss", str(AUDIO_TRIM_START),
...
def main():
    print(f"v43 brand-ad-thumb · {FPS}fps · {TOTAL_S}s · held thumbnail + audio offset {AUDIO_OFFSET_S}s")
