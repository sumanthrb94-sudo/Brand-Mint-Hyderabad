"""
Brand Mint Studios — Brand Story Ident (v28 / symmetric bubbles)

22.0s, 60fps, 1080x1920 vertical · production-agency grade.

Changes from v27:
- Reverted from triangle-pyramid particles back to DOT BUBBLES (circles
  with specular highlight + outer glow halo). Per user direction.
- LATTICE EXPANDED to cover the canvas symmetrically — no more empty
  corners in shot 3. Grid is now 18 cols x 22 rows with tighter
  spacing, so it spans roughly 1000x1200px centered on the mark.
  Particle count 200 -> 396 to match the larger grid.
- M target set expanded to 396 points along the M path so each pyramid
  has its own clean target (no clustering, no jitter — clean symmetric
  chain).
- All other v27 improvements retained: symmetric 120deg orbiters,
  no motion trails, no kicker text.

SHOT LIST (text-free until shot 6):
  1.  0.0- 3.0s  THE VOID         4 dots · seed + 3 orbiters at 120deg
  2.  3.0- 5.5s  THE BREAK        diagonal LL->UR · 14 stream dots
  3.  5.5- 9.5s  THE ARCHITECTURE 396-dot 3D lattice (fills canvas)
  4.  9.5-13.5s  THE COALESCE     dots -> clean symmetric M chain
...
OUT_DIR = HERE / "out"
OUT_MP4 = OUT_DIR / "brandmint-bubbles-60fps.mp4"
...
def main():
    print(f"v28 bubbles-symmetric · {FPS}fps · {TOTAL_S}s · {N_PARTICLES} dots")
    render_all_frames()
    mux()
