"""
Brand Mint Studios — Brand Story Ident (v39 / tidy lattice)
...
OUT_DIR = HERE / "out"
OUT_MP4 = OUT_DIR / "brandmint-tidylattice-60fps.mp4"
...
# 14 x 28 lattice = 392 slots, matches N_M_PARTICLES (= N_PARTICLES-N_LEAD).
# No orphan corner positions — every lattice slot gets a unique particle.
# Spacing 64x50 -> spans ~896x1400 px centered on mark.
N_LATTICE_COLS = 14
N_LATTICE_ROWS = 28
N_PARTICLES = 4 + N_LATTICE_COLS * N_LATTICE_ROWS   # = 396 (4 lead + 392 m-bound)
...
def hook_gap_bbox(t: float):
    ...
    # Hook 2 window 5.75 - 9.55 — text is widest here ("THEY'RE architected.")
    if 5.55 <= t <= 9.80:
        gap_a = smoothstep(5.75, 6.20, t) * (1 - smoothstep(9.20, 9.70, t))
        if gap_a > 0.01:
            return (CENTER_X - 560, HOOK_Y - 90, CENTER_X + 560, HOOK_Y + 70, gap_a)
...
def main():
    print(f"v39 tidylattice · {FPS}fps · {TOTAL_S}s · 14x28 lattice + wider gap + Hook3 +0.5s")
