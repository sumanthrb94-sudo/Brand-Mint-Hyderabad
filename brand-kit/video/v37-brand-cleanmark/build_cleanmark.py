"""
Brand Mint Studios — Brand Story Ident (v37 / clean mark)
...
OUT_DIR = HERE / "out"
OUT_MP4 = OUT_DIR / "brandmint-cleanmark-60fps.mp4"
...
def render_hook(t: float) -> str:
    """Three animated hook overlays in the pre-reveal phase.
    All positioned in the lower 5th of the canvas (y ~ 1670) so they
    sit beneath the lattice/mark without competing for the eye."""
    y_anchor = 1660
    parts = []
    ...
    if 1.90 <= t <= 5.50:
        ...
        a = appear * leave
        if a > 0.01:
            # Letter-spacing settle for that "cascading-in" feel
            ls = lerp(0.30, 0.10, ease_out_cubic(smoothstep(0.0, 0.4, local)))
            yoff = lerp(22, 0, ease_out_cubic(smoothstep(0.0, 0.35, local)))
            parts.append(
    ...
    # ── Hook 2: THEY'RE  ARCHITECTED.  (6.0-9.4s, mint emphasis)
    if 5.75 <= t <= 9.55:
        ...
        if a1 > 0.01 or a2 > 0.01:
            # Centered two-word layout:
            #   "THEY'RE"      ends at  CENTER_X - 18
            #   "architected." starts at CENTER_X + 18
            # → 36px visible gap between the words, no overlap.
            gap = 18
            tx_off = lerp(-22, 0, ease_out_cubic(smoothstep(0.05, 0.40, local)))
            # "THEY'RE" — slides in from left, RIGHT-aligned to center
            parts.append(
                f'<text x="{CENTER_X - gap + tx_off:.1f}" y="{y_anchor + 10:.1f}" '
            ...
            # "architected." — LEFT-aligned to the center, mint gradient, scale-punch
            sp = 1.0 + 0.07 * math.sin(min(1.0, max(0, (local - 0.40) * 4)) * math.pi)
            parts.append(
                f'<g transform="translate({CENTER_X + gap:.1f} {y_anchor + 10:.1f}) scale({sp:.3f})" '
                f'opacity="{a2:.3f}">'
                f'<text x="0" y="0" font-family="{FONT_DISPLAY}" font-weight="800" '
    ...
    # ── Hook 3: ONE PRINCIPLE AT A TIME.  (9.8-13.0s)
    if 9.60 <= t <= 13.10:
        local = localT(t, 9.60, 13.00)
        appear = smoothstep(0.05, 0.30, local)
        leave  = 1 - smoothstep(0.85, 1.00, local)
        a = appear * leave
        if a > 0.01:
            ls = lerp(0.30, 0.10, ease_out_cubic(smoothstep(0.0, 0.45, local)))
            yoff = lerp(22, 0, ease_out_cubic(smoothstep(0.0, 0.40, local)))
            parts.append(
                f'<text x="{CENTER_X}" y="{y_anchor + yoff:.1f}" '
            ...
            # Mint accent dot beside "PRINCIPLE"
...
def svg_for_frame(f: int) -> str:
  ...
  <!-- Hook overlays (pre-reveal, lower-5th, animated) -->
  {render_hook(t)}
...
def main():
    print(f"v37 cleanmark · {FPS}fps · {TOTAL_S}s · M highlight suppression from COALESCE start")
