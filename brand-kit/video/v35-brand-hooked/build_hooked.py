"""
Brand Mint Studios — Brand Story Ident (v35 / hooked intro)
...
OUT_DIR = HERE / "out"
OUT_MP4 = OUT_DIR / "brandmint-hooked-60fps.mp4"
...
def render_hook(t: float) -> str:
    ...
    if 5.75 <= t <= 9.55:
        ...
        if a1 > 0.01 or a2 > 0.01:
            # "THEY'RE" — slides in from left
            tx_off = lerp(-30, 0, ease_out_cubic(smoothstep(0.05, 0.40, local)))
            parts.append(
                f'<text x="{CENTER_X - 240 + tx_off:.1f}" y="{y_anchor + 10:.1f}" '
                f'font-family="{FONT_DISPLAY}" font-weight="600" font-size="60" '
                f'letter-spacing="0.04em" fill="{PAPER}" font-style="italic" '
                f'text-anchor="start" opacity="{a1:.3f}">THEY&#39;RE</text>'
            )
            # "ARCHITECTED." — mint gradient, scale punch at local~0.55
            sp = 1.0 + 0.07 * math.sin(min(1.0, max(0, (local - 0.40) * 4)) * math.pi)
            parts.append(
                f'<g transform="translate({CENTER_X - 50:.1f} {y_anchor + 10:.1f}) scale({sp:.3f})" '
                f'opacity="{a2:.3f}">'
                f'<text x="0" y="0" font-family="{FONT_DISPLAY}" font-weight="800" '
...
def main():
    print(f"v35 hooked · {FPS}fps · {TOTAL_S}s · {N_PARTICLES} dots · 3 hooks + chyrons + shockwaves")
