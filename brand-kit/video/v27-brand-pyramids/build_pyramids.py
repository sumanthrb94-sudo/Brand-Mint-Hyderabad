"""
Brand Mint Studios — Brand Story Ident (v27 / pyramid cut)

22.0s, 60fps, 1080x1920 vertical · production-agency grade.

Changes from v26:
- Particles are now SHADED TRIANGLE PYRAMIDS instead of soft circles.
  Each pyramid renders as 2 visible faces (right lit, left shadowed)
  separated by an apex-to-base centerline + a thin outline stroke for
  crispness. Apex-up orientation for every particle (perfect symmetry
  across the field).
- All particle/path randomness removed:
    * Perpendicular jitter on M targets removed -> clean symmetric M
    * 3 orbiters at PERFECTLY 120-degree spacing, equal radius/speed
    * Pyramid orientation never rotates (always apex up)
- Motion trails removed — agency cleanliness over motion-blur noise.
- Depth grading still active (close pyramids brighter, far darker)
  but applied to face fills rather than circle fills.

SHOT LIST (text-free until shot 6):
  1.  0.0- 3.0s  THE VOID         4 pyramids · 1 center + 3 orbiters at 120deg
  2.  3.0- 5.5s  THE BREAK        diagonal LL->UR · 14 stream pyramids
  3.  5.5- 9.5s  THE ARCHITECTURE 200-pyramid 3D lattice
  4.  9.5-13.5s  THE COALESCE     pyramids -> bold M outline
...
OUT_DIR = HERE / "out"
OUT_MP4 = OUT_DIR / "brandmint-pyramids-60fps.mp4"
...
# Bumped from 90 -> 200 for cinematic density + bolder M coalesce
N_LATTICE_COLS = 14
N_LATTICE_ROWS = 14
N_PARTICLES = N_LATTICE_COLS * N_LATTICE_ROWS   # 196 ~ 200
...
def make_particles() -> List[P]:
    ...
    # 3 orbiters with different radii, phases, speeds, depths
    for k in range(N_ORBITERS):
        ps.append(P(
            x=seed_x, y=seed_y,
            z=lerp(0.4, -0.1, k / max(1, N_ORBITERS - 1)),
            vx=0, vy=0, vz=0,
            tx=seed_x, ty=seed_y,
            born_t=0.15 + k * 0.2,
            role="orbiter",
    ...
    for i in range(n_core):
        lx, ly, lz = LATTICE[i % len(LATTICE)]
        ang = random.uniform(0, math.tau)
        rad = random.uniform(60, 380)
        born = 5.55 + (i / max(1, n_core - 1)) * 2.8
        ps.append(P(
...
def step(t: float):
    ...
    for i, p in enumerate(PARTICLES):
        ...
        if p.role == "orbiter":
            # SHOT 1: orbit the seed at PERFECTLY SYMMETRIC 120deg spacing,
            # equal radius, equal angular velocity. Indices 0..N_ORBITERS-1
            # encoded in p.seed (we set seed = k/N_ORBITERS at init).
            if t < 3.0:
                age = t - p.born_t
                appear = ease_out_cubic(clamp01(age / 0.5))
                ORBIT_RADIUS = 56
                ORBIT_W = 0.55  # rad/s — single shared angular velocity
                radius = ORBIT_RADIUS * appear
                # Phase = seed * 2*pi, giving exactly 120deg intervals for 3 orbiters
                phase = p.seed * math.tau
                ang = ORBIT_W * t + phase
                # All orbiters at the same z for symmetric brightness
                p.z = 0.15
                p.x = THIRD_X_LEFT + math.cos(ang) * radius
                p.y = THIRD_Y_BOT + math.sin(ang) * radius
            else:
                # Migrate to a lattice slot during shots 2/3, then M target
                idx = (i + 5) % len(LATTICE)
                if t < 9.5:
...
def render_particles(t: float, alpha_mul: float = 1.0) -> str:
    ...
    for i, p in enumerate(PARTICLES):
        ...
        s, a, glow_r = proj(p.z)
        # Pyramid "size" = half-base-width-in-pixels. Increased for visual
        # presence — pyramids read smaller than circles of equal area.
        size = 8.4 * s * bloom
        # Color by depth: close = brightest faces, far = deepest faces.
        # face_light  = the lit (right) face — toward upper-right key light
        # face_dark   = the shadowed (left) face
        # face_outline = thin stroke for crispness
        if p.z > 0.2:
            face_light, face_dark, face_outline = MINT_1, MINT_3, MINT_4
        elif p.z > -0.2:
            face_light, face_dark, face_outline = MINT_2, MINT_3, MINT_4
        else:
            face_light, face_dark, face_outline = MINT_3, MINT_4, INK
        a *= alpha_mul
        if a <= 0.02: continue
        ...
        if p.role == "lead" and t < 3.0:
            ...
            size *= pulse
        # Outer glow halo (still circular — it's volumetric light scatter)
        total_glow = glow_r + seed_glow_extra
        if total_glow > 0.5:
            parts.append(
                f'<circle cx="{p.x:.1f}" cy="{p.y:.1f}" r="{size * 1.6 + total_glow:.1f}" '
                f'fill="url(#particleGlow)" opacity="{a * 0.50:.3f}"/>'
            )
        # PYRAMID BODY — equilateral triangle, apex up, 2 visible shaded
        # faces split by an apex-to-base centerline. Squash-and-stretch is
        # applied as a vertical scale around the centroid via SVG transform.
        cx_, cy_ = p.x, p.y
        # Apex slightly above the centroid so the centroid sits at (cx_, cy_)
        # Equilateral triangle with side = 2*size*0.866; apex-to-base distance = 1.5*size
        apex_y_off  = -size              # apex y offset from centroid
        base_y_off  = size * 0.5         # base midline y offset from centroid
        half_base   = size * 0.866       # half of base width
        # Pixar squash: scale Y around centroid
        if stretch_y > 1.005:
            t_apex_y = cy_ + apex_y_off * stretch_y
            t_base_y = cy_ + base_y_off * stretch_y
        else:
            t_apex_y = cy_ + apex_y_off
            t_base_y = cy_ + base_y_off
        ap = (cx_, t_apex_y)
        bl = (cx_ - half_base, t_base_y)
        br = (cx_ + half_base, t_base_y)
        bm = (cx_,             t_base_y)    # centerline base point
        # Right face (lit) — apex → base-right → base-mid
        parts.append(
            f'<polygon points="{ap[0]:.1f},{ap[1]:.1f} {br[0]:.1f},{br[1]:.1f} {bm[0]:.1f},{bm[1]:.1f}" '
            f'fill="{face_light}" opacity="{a:.3f}"/>'
        )
        # Left face (shadow) — apex → base-mid → base-left
        parts.append(
            f'<polygon points="{ap[0]:.1f},{ap[1]:.1f} {bm[0]:.1f},{bm[1]:.1f} {bl[0]:.1f},{bl[1]:.1f}" '
            f'fill="{face_dark}" opacity="{a:.3f}"/>'
        )
        # Crisp outer outline — thin stroke around the silhouette
        if size > 3.5:
            parts.append(
                f'<polygon points="{ap[0]:.1f},{ap[1]:.1f} {br[0]:.1f},{br[1]:.1f} {bl[0]:.1f},{bl[1]:.1f}" '
                f'fill="none" stroke="{face_outline}" stroke-width="0.7" '
                f'stroke-linejoin="round" opacity="{min(1.0, a * 0.85):.3f}"/>'
            )
    return "".join(parts)
...
def main():
    print(f"v27 pyramids · {FPS}fps · {TOTAL_S}s · {N_PARTICLES} pyramids")
    render_all_frames()
    mux()
