"""
Brand Mint Studios — Brand Story Ident (v36 / cleanup pass)
...
OUT_DIR = HERE / "out"
OUT_MP4 = OUT_DIR / "brandmint-cleanup-60fps.mp4"
...
def render_particles(t: float, alpha_mul: float = 1.0) -> str:
    ...
    for i, p in enumerate(PARTICLES):
        ...
        # Suppress for the M-locked phase — when 392 dots stack into the
        # M chain, 392 small bright highlights read as white pinpricks.
        # The 4 lead/orbiter anchor dots always keep their highlights so
        # they continue to read as distinct bubbles at the edges.
        is_locked_m = (p.role in ("core", "stream")) and t >= 13.0
        if p.z > -0.3 and r > 2.5 and not is_locked_m:
            hx = p.x + r * 0.35
            hy = p.y - r * 0.35
            hr = r * 0.30
            parts.append(
...
def main():
    print(f"v36 cleanup · {FPS}fps · {TOTAL_S}s · square lattice + no M highlights + Hook 2 split")
