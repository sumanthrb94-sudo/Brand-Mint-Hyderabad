...
# (splash phrases, project names, descriptors) must call fit_to_width().
SAFE_TEXT_W = 920
...
def render_splash(beat: Beat, local: float, scale: float) -> str:
    ...
    line1_pt = fit_to_width(beat.text,  SAFE_TEXT_W, start_pt=100, kind="serif")
    line2_pt = fit_to_width(beat.text2, SAFE_TEXT_W, start_pt=100, kind="serif")
    big_pt_base = fit_to_width("MODCON", SAFE_TEXT_W, start_pt=132, kind="bold")
