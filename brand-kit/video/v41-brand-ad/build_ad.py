"""
Brand Mint Studios — Brand Ad (v41 / social marketing + Meta ads)

15.0s · 60fps · 1080x1920 vertical · DIFFERENT STYLE FROM V40.
...
OUT_DIR = HERE / "out"
OUT_MP4 = OUT_DIR / "brandmint-ad-15s-60fps.mp4"
...
def scene_local(t: float, scene_i: int):
    """Return local time (0..1) within a scene window, plus the 0..2.5s
    raw local time. Each scene runs from i*2.5 to (i+1)*2.5."""
    start = scene_i * SCENE_DURATION
    end   = (scene_i + 1) * SCENE_DURATION
    if t < start - 0.01 or t > end + 0.01:
        return None, None
    local_raw = t - start
    local_norm = clamp01(local_raw / SCENE_DURATION)
...
def scene_bg(t: float) -> str:
    ...
    if scene_i >= N_SCENES: scene_i = N_SCENES - 1
...
def svg_for_frame(f: int) -> str:
    t = f / FPS
    # Tiny micro-flash on scene boundary for a tactile cut feel
    flash = ""
    for boundary in [2.5, 5.0, 7.5, 10.0, 12.5]:
        delta = t - boundary
        if -0.02 < delta < 0.08:
...
def mux():
    """Mux frames into a silent MP4. Audio is added later by the user."""
...
def main():
    print(f"v41 brand-ad · {FPS}fps · {TOTAL_S}s · 6 scenes · NO AUDIO YET")
    render_all_frames()
    mux()
    size_mb = OUT_MP4.stat().st_size / 1024 / 1024
    print(f"  done → {OUT_MP4} ({size_mb:.2f}MB)")
    print("\nTo add audio after upload:")
    print("  ffmpeg -i brandmint-ad-15s-60fps.mp4 -i your-music.mp3 \\")
    print("    -c:v copy -c:a aac -b:a 192k -shortest \\")
    print("    -af 'afade=t=out:st=14.5:d=0.5' \\")
    print("    -movflags +faststart \\")
    print("    brandmint-ad-15s-final.mp4")
