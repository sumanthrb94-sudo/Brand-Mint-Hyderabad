"""
Brand Mint Studios — Brand Story Ident (v38 / eye-level + option C)
...
OUT_DIR = HERE / "out"
OUT_MP4 = OUT_DIR / "brandmint-eyelevel-60fps.mp4"
        ...
        if hook_bbox is not None and p.role in ("core", "stream"):
            ...
            if gx0 <= p.x <= gx1 and gy0 <= p.y <= gy1:
                ...
                edge_norm = max(abs(p.x - bx_c) / half_w,
                                abs(p.y - by_c) / half_h)
                # Particles deep inside (edge_norm < 0.8) get fully hidden
                inside_factor = 1 - smoothstep(0.80, 1.00, edge_norm)
...
def hook_gap_bbox(t: float):
    ...
    active hook, or None. The alpha controls fade-in/fade-out of the gap
    so the bubble field opens and closes smoothly rather than popping.
    """
    # Hook 1 window 1.90 - 5.50
    if 1.50 <= t <= 5.80:
        gap_a = smoothstep(1.70, 2.20, t) * (1 - smoothstep(5.20, 5.70, t))
        if gap_a > 0.01:
            return (CENTER_X - 460, HOOK_Y - 70, CENTER_X + 460, HOOK_Y + 50, gap_a)
    # Hook 2 window 5.75 - 9.55
    if 5.55 <= t <= 9.80:
        gap_a = smoothstep(5.75, 6.20, t) * (1 - smoothstep(9.20, 9.70, t))
        if gap_a > 0.01:
            return (CENTER_X - 480, HOOK_Y - 80, CENTER_X + 480, HOOK_Y + 60, gap_a)
    # Hook 3 window 9.60 - 10.80 (closes earlier so M can form cleanly)
    if 9.40 <= t <= 10.90:
        gap_a = smoothstep(9.55, 9.90, t) * (1 - smoothstep(10.40, 10.80, t))
        if gap_a > 0.01:
            return (CENTER_X - 430, HOOK_Y - 70, CENTER_X + 430, HOOK_Y + 50, gap_a)
    return None
...
def render_hook(t: float) -> str:
    ...
    # ── Hook 3: ONE PRINCIPLE AT A TIME.  (9.6-10.8s, shorter so gap
    #     closes before M chain arrives at center)
    if 9.60 <= t <= 10.85:
        local = localT(t, 9.60, 10.80)
...
def main():
    print(f"v38 eyelevel · {FPS}fps · {TOTAL_S}s · hooks at eye-level + bubble gap + multi-beam + flare")
