"""
Brand Mint Studios — Brand Story Ident (v33 / tightened)

19.5s, 60fps, 1080x1920 vertical · production-agency grade.

Change from v32: the 2.5s "breath" phase between the edge scatter
and the lattice spawn has been removed entirely. The lattice now
starts spawning at t=3.0s (instead of t=5.5s). Every subsequent
phase boundary has shifted by -2.5s to match. Total length
22.0s -> 19.5s.

Phase map:
   0.0- 3.0s   THE VOID         4 dots scatter to edges
   3.0- 7.0s   THE ARCHITECTURE lattice spawns center-out
   7.0- 9.5s   THE COALESCE     smooth flow into M
   9.5-11.0s   M LOCKED         settle, breath
  11.0-14.0s   THE LANDING      god-ray, disc fill, stroke draw
  14.0-17.0s   THE NAME         wordmark + tagline
  17.0-19.5s   THE SIGNATURE    URL + fade to ink

Single change from v31: the 14-dot diagonal stream removed entirely.
Shot 2 is now a quiet breath — only the 4 edge anchors holding while
the atmospheric haze drifts — before the lattice begins spawning in
shot 3 at t=5.5s. All 392 non-anchor dots are now in a single core
pool that goes directly from off-screen origin -> lattice -> M.

Changes from v30:
- 4 LEAD dots stay PERMANENTLY at the four edges after shot 1. They
  become decorative anchors framing the composition. No migration
  back to the lattice or to the M.
- M target assignment FIXED: the 14 stream + 378 core particles
  (= 392 total) are now ALL proximity-matched to exactly 392 M-path
  points. No more unclaimed M targets, no more index-vs-proximity
  conflicts. Every M position is filled by a unique dot.
- Smoother COALESCE: spring k 0.10 -> 0.075, perpendicular curve
  bias removed (straight flow into M targets, no oscillating arcs).
  Cuts the "shooting bubbles" feel.
- Earlier M lock: COALESCE -> LOCK boundary moved from 13.5s to
  12.0s. Gives the locked symmetric M 1.5s of breathing room
  before LANDING starts at 13.5s.
- M dot radius bumped 6.8 -> 8.5 for stronger visual weight.

Changes from v29:
- Shot 1 motion replaced: the 4 dots now START near the canvas center
  and slowly SCATTER outward to the FOUR EDGES (top, right, bottom,
  left). Each dot gets its own edge anchor on the cross axis pattern.
- COALESCE compressed and stiffened so the M crystallizes by ~11.5s
  instead of 13.5s, giving the locked mark a longer breathing window
  before LANDING. Spring constant up from 0.065 to 0.10.
- M particles snap z=0 at lock-on -> all dots in the final M are
  identical size for clean symmetric weight.
...
OUT_DIR = HERE / "out"
OUT_MP4 = OUT_DIR / "brandmint-tight-60fps.mp4"
...
def make_particles() -> List[P]:
    ...
    for rank, (slot_i, _) in enumerate(lattice_dists):
        u = rank / max(1, n_m - 1)
        birth_for_slot[slot_i] = 3.05 + u * 2.8
...
def step(t: float):
    ...
    for i, p in enumerate(PARTICLES):
        ...
        if p.role in ("lead", "orbiter"):
            ...
            if t < 3.0:
                age = t - p.born_t
                appear = ease_out_cubic(clamp01(age / 2.4))
        ...
        if t < 3.0:
            continue
        if t < 7.0:
            # ARCHITECTURE (3.0-7.0): spring to lattice slot, bob
            ...
            # COALESCE (7.0-9.5): smooth flow into M target
            k = 0.075 * smoothstep(7.0, 8.0, t); damp = 0.85
            ...
            # LOCK (t >= 9.5): hard settle on M, z=0
            p.x = lerp(p.x, p.tx, 0.45)
            p.y = lerp(p.y, p.ty, 0.45)
            p.z = lerp(p.z, 0.0, 0.30)
            p.vx *= 0.3; p.vy *= 0.3; p.vz *= 0.3
...
def render_shot_text(t: float) -> str:
    ...
    # ── BRAND MINT wordmark (14.0 - 16.0)
    if t > 14.0:
        local = localT(t, 14.0, 16.0)
...
def render_atmosphere(t: float) -> str:
    ...
    for i in range(5):
        ...
        if t > 10.5:
            base_a *= 1.0 - smoothstep(10.5, 13.0, t) * 0.6
        parts.append(
...
def render_god_ray(t: float, cx: float, cy: float) -> str:
    ...
    # Active 10.5-15.0s, peak at ~12.5s
    if t < 10.5 or t > 15.0: return ""
    p = localT(t, 10.5, 12.5) * (1 - localT(t, 13.5, 15.0))
    p = max(0.0, p)
    if p < 0.02: return ""
...
def camera_state(t: float) -> Tuple[float, float, float]:
    """Returns (scale, tx, ty). v33 timing — 19.5s total."""
    if t < 3.0:
        # SHOT 1 (0-3): push-in 2%
        p = ease_in_out_cubic(t / 3.0)
        return (1.0 + p * 0.02, 0, 0)
    if t < 7.0:
        # ARCHITECTURE (3-7): pull-back -4%
        p = ease_in_out_cubic(localT(t, 3.0, 7.0))
        return (lerp(1.02, 0.96, p), 0, 0)
    if t < 11.0:
        # COALESCE + LOCK (7-11): drift 0.96 -> 0.98, then start push
        p = ease_out_cubic(localT(t, 7.0, 11.0))
        return (lerp(0.96, 0.98, p), 0, 0)
    if t < 14.0:
        # LANDING (11-14): push-in +5%
        p = ease_in_out_cubic(localT(t, 11.0, 14.0))
        return (lerp(0.98, 1.05, p), 0, 0)
    if t < 17.0:
        # NAME (14-17): ease back to 1.0
        p = ease_out_cubic(localT(t, 14.0, 17.0))
        return (lerp(1.05, 1.0, p), 0, 0)
    return (1.0, 0, 0)

# ---------- Vignette (breathes per beat) ----------
def vignette_strength(t: float) -> float:
    if t < 3.0:  return 0.70     # tight on the void
    if t < 7.0:  return 0.40     # widest on architecture
    if t < 11.0: return 0.48     # narrowing on coalesce/lock
    if t < 14.0: return 0.65     # tighter on landing
    if t < 17.0: return 0.55     # neutral on name
...
def svg_for_frame(f: int) -> str:
    ...
    # Mark animation timing — 19.5s tightened cut
    mark_draw = ease_out_cubic(localT(t, 10.0, 12.5))   # stroke draws on
    mark_disc = ease_out_cubic(localT(t, 12.0, 14.0))   # disc fills
    mark_a    = ease_out_cubic(localT(t, 9.5, 11.0))    # M materializes
    # Soft glow pulse on landing
    glow = 0.0
    if t > 12.5:
        p = clamp01((t - 12.5) / 1.8)
        glow = (1 - (1 - p) ** 3) * (1 - p) * 0.85

    # Mark rises into upper-mid for the NAME phase
    rise = lerp(0, -160, ease_out_cubic(localT(t, 14.0, 15.5)))
    mark_cy = MARK_CY + rise
    mark_scale = lerp(560, 320, ease_out_cubic(localT(t, 14.0, 15.5)))

    # Particle alpha — fade out as mark dominates
    p_alpha = 1.0 - smoothstep(10.0, 11.5, t)

    # Final fade
    fade_t = localT(t, 19.0, 19.5)
...
def main():
    print(f"v33 tightened · {FPS}fps · {TOTAL_S}s · {N_PARTICLES} dots · 4 anchor + 392 to M")
    render_all_frames()
    mux()
