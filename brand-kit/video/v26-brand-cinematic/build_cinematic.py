"""
Brand Mint Studios — Brand Story Ident (v26 / cinematic cut)

22.0s, 60fps, 1080x1920 vertical · dense + bold + atmospheric.

Pixar-style movement (anticipation, squash-and-stretch on births) +
Dolby Atmos-tier depth (atmospheric haze layers, god-ray during reveal).

Changes from v25:
- 4 particles in THE VOID (seed + 3 orbiters at different radii/speeds)
- N_PARTICLES doubled 90 -> 200 — denser presence
- 2 particles cluster per M target with small offset -> THICK BOLD M
- Particle radii increased ~50% for bolder dots
- Atmospheric haze: 5 large soft mint-fog circles drift slowly in BG
- God-ray light beam appears during THE LANDING
- Squash-and-stretch (1.4x Y-stretch on birth, settles in 0.3s)
- All non-brand-reveal text removed (no kicker chyrons during shots 3-4)
- Brand-reveal text only: BRAND MINT wordmark + tagline + signature

SHOT LIST (text-free until shot 6):
  1.  0.0- 3.0s  THE VOID         4 particles in slow orbit
  2.  3.0- 5.5s  THE BREAK        diagonal LL->UR · 14 stream particles
  3.  5.5- 9.5s  THE ARCHITECTURE 200-particle 3D lattice with bob
  4.  9.5-13.5s  THE COALESCE     particles -> dense bold M outline
...
OUT_DIR = HERE / "out"
OUT_MP4 = OUT_DIR / "brandmint-cinematic-60fps.mp4"
...
# Build a denser, BOLDER M target set: walk the path with N_PARTICLES sample
# points, then jitter each by a small perpendicular offset (-12..+12px) so
# the resulting "stroke" is thick rather than a thin 1-particle line.
def m_targets_bold(n: int, cx: float, cy: float, scale: float) -> List[Tuple[float,float]]:
    base = m_path_targets(n, cx, cy, scale)
    out = []
    random.seed(99)
    for i, (x, y) in enumerate(base):
        if i + 1 < len(base):
            dx, dy = base[i+1][0] - x, base[i+1][1] - y
        else:
            dx, dy = base[i][0] - base[i-1][0], base[i][1] - base[i-1][1]
        L = math.hypot(dx, dy) + 1e-6
        # Perpendicular unit vector
        nx, ny = -dy / L, dx / L
        offset = random.uniform(-13, 13)
        out.append((x + nx * offset, y + ny * offset))
    return out

M_TARGETS = m_targets_bold(N_PARTICLES, CENTER_X, MARK_CY, scale=560)
...
def step(t: float):
    ...
    for i, p in enumerate(PARTICLES):
        ...
        if p.role == "orbiter":
            # SHOT 1: orbit the seed at different radii / speeds / depths
            if t < 3.0:
                age = t - p.born_t
                # Anticipation: scale out from radius 0 over 0.5s
                appear = ease_out_cubic(clamp01(age / 0.5))
                radius = lerp(0, 38 + p.seed * 30, appear)
                # Each orbiter has a different angular speed + phase
                w = 0.55 + p.seed * 0.45   # rad/s
                phase = p.seed * math.tau
                ang = w * t + phase
                # Slight z-bob for depth feel
                p.z = (p.seed - 0.5) * 0.6 + math.sin(t * 0.8 + p.seed * 3) * 0.15
                p.x = THIRD_X_LEFT + math.cos(ang) * radius
                p.y = THIRD_Y_BOT + math.sin(ang) * radius * 0.78
            else:
...
def render_particles(t: float, alpha_mul: float = 1.0) -> str:
    ...
    for i, p in enumerate(PARTICLES):
        ...
        s, a, glow_r = proj(p.z)
        # Base radius bumped from 4.6 -> 6.8 for thicker, bolder particles
        r = 6.8 * s * bloom
        ...
        # Body — ellipse during squash-and-stretch phase, circle once settled
        if stretch_y > 1.02:
            parts.append(
...
def main():
    print(f"v26 cinematic · {FPS}fps · {TOTAL_S}s · {N_PARTICLES} particles")
    render_all_frames()
    mux()
