"""
Brand Mint Studios — Brand Story Ident (v32 / no stream)
...
OUT_DIR = HERE / "out"
OUT_MP4 = OUT_DIR / "brandmint-nostream-60fps.mp4"
...
def step(t: float):
    ...
    for i, p in enumerate(PARTICLES):
        ...
        if t < 9.5:
            ...
            # SHOT 4 (COALESCE): SMOOTH flow into proximity-matched M
            # target. No perpendicular bias — straight spring approach.
            # Lighter spring (0.075) so it feels like flow not a shot.
            k = 0.075 * smoothstep(9.5, 10.5, t); damp = 0.85
            dx, dy = p.tx - p.x, p.ty - p.y
            p.vx = (p.vx + dx * k) * damp
            p.vy = (p.vy + dy * k) * damp
            # Snap z toward 0 for uniform M dot size
            p.vz = (p.vz + (-p.z * 0.18)) * 0.88
            p.x += p.vx; p.y += p.vy; p.z += p.vz
        else:
            # LOCK (t >= 12.0): hard settle onto M target, z=0
...
def main():
    print(f"v32 no-stream · {FPS}fps · {TOTAL_S}s · {N_PARTICLES} dots · 4 anchor + 392 to M")
    render_all_frames()
    mux()
